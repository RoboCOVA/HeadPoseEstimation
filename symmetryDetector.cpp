...
SymmetryDetector::SymmetryDetector(const Size image_size, const Size hough_size, const int rot_resolution) {
	...
	this->diagonal = hypot(imageSize.width, imageSize.height);
...
void SymmetryDetector::vote(Mat& image, int min_pair_dist, int max_pair_dist) {
	...
	//int numrho;
	//long sizeaccum;
	//accumulator = new int[maxthetha * numrho];

	//sizeaccum = sizeof(int)*maxthetha*numrho;
	//memset(accumulator, 0, sizeaccum);
//=======================================================================================================
		...
		for (i = 0; i < rhoDivision; i++) {
			...
			for (float* x0 = col_start; x0 != col_end - 1; x0++) {
				for (float* x1 = x0 + 1; x1 != col_end; x1++) {
					...
					//int rho_index = [rhoMax*uh + t]++;
					//180*(rhoMax +(int)(*x0+*x1));
					//if (rho_index<10)
					accum_ptr[rho_index]++;
					//cout << accum_ptr[rho_index]<<"\n";
...
*/
Mat SymmetryDetector::getAccumulationMatrix(float thresh) {
//delete[] reRows;
	Mat thresholded;
...
**/
vector<pair<Point, Point>> SymmetryDetector::getResult(int no_of_peaks, float threshold) {
